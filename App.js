...
import turinLocations from './assets/puntiDraccolta'; // i dati dei centri di raccolta

// Definiamo il componente principale che mostra la lista dei punti di raccolta
export default function App() {
  const [centriRaccolta, setCentriRaccolta] = useState([...turinLocations]); // la lista dei centri di raccolta
  ...
  useEffect(() => {
    if (location) {
      ...
      // ricava la stringa con le coordinate dell'utente
      const start =
      ...
      text.current = 'GPS connected: your location(' + start + ')';
        ...
        fetch(URL) // eseguo l'accesso alle API
          ...
          .then((data) => {
            ...
            if (data.rows.length > 0) {
              ...
              newCentriRaccolta[index].destinationAddr =
                data.destination_addresses[0]; // memorizzo l'indirizzo del centro di raccolta
            }
          })
